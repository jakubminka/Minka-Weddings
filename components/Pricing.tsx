      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {PRICING_PACKAGES.map((pkg, idx) => (
            <div 
              key={idx} 
              className={`relative p-10 rounded-3xl border transition-all duration-500 hover:shadow-2xl flex flex-col ${
                pkg.recommended 
                ? 'border-ochre shadow-xl scale-105 z-10 bg-stone-50' 
                : 'border-stone-100 bg-white'
              }`}
            >
              {pkg.recommended && (
                <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-ochre text-white px-6 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-[0.2em]">
                  Nejoblíbenější
                </div>
              )}
        ...
        <div className="mt-16 text-center text-stone-400 text-xs italic">
          * Neobsahuje cestovné (8 Kč/km z Prahy nebo Ostravy). Možnost individuální úpravy balíčků na míru.
        </div>
      </div>
    </section>
  );
